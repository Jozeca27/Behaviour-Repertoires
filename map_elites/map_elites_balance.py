...
class MAPElites:
    ...
    def add_to_grid(
        ...
        grid_idx = tuple(map(int, grid_idx))
        ...
        if (
            ...
            old_selection_count = (
                self.grid[grid_idx]["selection_count"]
                if grid_idx in self.grid
    ...
    def random_genome(self):
        ...
        return RecurrentTorqueHexapodController.random_genome_scales()
        #return RecurrentTorqueHexapodController.random_genome()

    def random_selection(self) -> np.ndarray:
        """
        Sample an elite with probability inversely proportional
        to how many times it has been selected.
        """
        ...
        # Higher weight for less frequently selected elites
        weights = np.array([
            1.0 / (self.grid[k]['selection_count'] + 1)
            for k in keys
        ], dtype=float)

        weights /= weights.sum()

        selected_key = keys[self.rng.choice(len(keys), p=weights)]
        selected_key = tuple(selected_key)  # enforce hashable key

        # Update selection counter
        self.grid[selected_key]['selection_count'] += 1
