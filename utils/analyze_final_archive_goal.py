...
Run from the repository root, for example:
    ...
    python -m utils.analyze_final_archive_goal \
        --archive SaveResults/Goal/Goal8DTrans/map_elites_final.pkl \
        --output bd_analysis_from_archive
"""
...
def load_reconstructed_records(trajectory_dir):
    records = []

    for file in sorted(Path(trajectory_dir).glob("cell_*.pkl")):
        ...
        records.append(
            ReconstructedTrajectory(
                ...
                archive_fitness=data["archive_fitness"],
                archive_bd=data["archive_bd"],
                genome=data["genome"],
                result=data["result"],
            )
        )
...
def reconstruct_archive_trajectories(
    ...
    try:
        for rank, (grid_idx, cell) in enumerate(items, start=1):
            result = evaluate_goal_controller_median(
                sim,
                cell["genome"],
            )
...
def evaluate_goal_controller_median(
    sim: HexapodTorqueSimulation,
    genome: np.ndarray,
) -> dict:
    """
    Evaluate a goal-conditioned controller on all configured goals and
    return the run whose fitness is the median.

    The returned dictionary has exactly the same structure as
    evaluate_controller(...), making it compatible with the existing
    analysis pipeline.
    """
    ...
    for goal in config.GOAL_VECTORS:
        ...
        run = dict(result)
        run["goal_vector"] = np.asarray(goal)
        runs.append(run)

    fitnesses = np.array(
        [r["fitness"] for r in runs],
        dtype=np.float32,
    )
...
def main() -> None:
    ...
    records = load_reconstructed_records(
        Path(args.output) / "reconstructed_trajectories"
    )
    print ("hii")
    ...
    print(f"Reconstructed {len(records)} archive cells from {args.archive}")
